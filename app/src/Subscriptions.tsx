...
function SubscribeToPlan() {
  ...
  const getSubscriptions = useCallback(async () => {
    if (user.data) {
      const subs = await fetchFromAPI('subscriptions', {
        method: 'GET',
      });
  ...
  return (
    <>
      <h2>Subscriptions</h2>
      <p>
        Subscribe a user to a recurring plan, process the payment, and
      ...
      <AuthCheck fallback={<SignIn />}>
        <div className="well">
          ...
          {user.data?.uid && <UserData userId={user.data.uid} />}
        </div>
        ...
        <div className="well">
          ...
          <button
            className={
              'btn ' +
              (plan === process.env.REACT_APP_PLAN_MONTHLY
                ? 'btn-primary'
                : 'btn-outline-primary')
            }
            onClick={() =>
              setPlan(process.env.REACT_APP_PLAN_MONTHLY)
            }
          >
            Choose Monthly $25/m
          </button>
          ...
          <button
            className={
              'btn ' +
              (plan === process.env.REACT_APP_PLAN_QUARTERLY
                ? 'btn-primary'
                : 'btn-outline-primary')
            }
            onClick={() =>
              setPlan(process.env.REACT_APP_PLAN_QUARTERLY)
            }
          >
